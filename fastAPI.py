...
def preprocess_data(data):
    # Appliquer l'encodage one-hot uniquement pour les colonnes spécifiées
    encoded_data = pd.get_dummies(data, columns=["Brand", "Location", "Fuel_Type", "Owner_Type"])

    # Sélectionner les colonnes à mettre à l'échelle
    columns_to_scale = ["Mileage", "Seats", "Year", "Kilometers_Driven", "Engine_numeric", "Power_numeric"]

    # Créer un objet StandardScaler
    scaler = StandardScaler()

    # Appliquer la mise à l'échelle sur les colonnes sélectionnées
...
async def get_brands(cursor: psycopg2.extensions.cursor = Depends(get_db)):
    ...
    cursor.execute("SELECT DISTINCT brand FROM vehicules ORDER BY brand ASC")
